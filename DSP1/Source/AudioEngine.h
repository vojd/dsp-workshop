...
public:
    ...
    AudioEngine()
    {
        for (size_t i = 0; i < maxNumVoices; ++i)
            addVoice (new Voice);  // everything required for playing a single note
        // useful if playing more notes than available voices
...
private:
    ...
    void renderNextSubBlock (AudioBuffer<float>& outputAudio, int startSample, int numSamples) override
    {
        ...
        auto audioBlock = juce::dsp::AudioBlock<float>(outputAudio);
        ...
        auto blockToUse = audioBlock.getSubBlock ((size_t) startSample, (size_t) numSamples);
